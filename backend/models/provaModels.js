const pool = require("../config/database");

function isUndefinedRelation(erro) {
  return erro && erro.code === "42P01";
}

async function listar(busca) {
  const params = [];
  let sql = "SELECT * FROM view_busca_questao_PES";

  if (busca) {
    params.push(`%${busca}%`);
    sql += " WHERE enunciado ILIKE $1";
  }

  sql += " ORDER BY id";

  try {
    const result = await pool.query(sql, params);
    return result.rows;
  } catch (erro) {
    if (!isUndefinedRelation(erro)) {
      throw erro;
    }
  }

  return listarSemView(busca);
}

async function listarSemView(busca) {
  const params = [];
  let sql = `
    SELECT
      questao.id,
      questao.enunciado,
      questao.explicacao,
      vestibular.nome AS vestibular,
      subtopico.nome AS subtopico,
      avaliacao.nivel
    FROM questao
    INNER JOIN vestibular
      ON questao.vestibular_id = vestibular.id
    INNER JOIN subtopico
      ON questao.subtopico_id = subtopico.id
    INNER JOIN avaliacao
      ON questao.avaliacao_id = avaliacao.id
  `;

  if (busca) {
    params.push(`%${busca}%`);
    sql += " WHERE questao.enunciado ILIKE $1";
  }

  sql += " ORDER BY questao.id";

  const result = await pool.query(sql, params);
  return result.rows;
}

async function montarPorDificuldade(nivel = "intermediario") {
  const result = await pool.query(
    `
    SELECT
      questao.id,
      questao.enunciado,
      avaliacao.nivel,
      alternativa.letra,
      alternativa.texto
    FROM questao
    INNER JOIN avaliacao
      ON questao.avaliacao_id = avaliacao.id
    INNER JOIN alternativa
      ON questao.id = alternativa.questao_id
    WHERE avaliacao.nivel = $1
    ORDER BY questao.id, alternativa.letra
    `,
    [nivel]
  );

  return result.rows;
}

async function montarPorVestibular(vestibular = "FUVEST") {
  const result = await pool.query(
    `
    SELECT
      questao.id,
      questao.enunciado,
      vestibular.nome,
      vestibular.ano,
      alternativa.letra,
      alternativa.texto
    FROM questao
    INNER JOIN vestibular
      ON questao.vestibular_id = vestibular.id
    INNER JOIN alternativa
      ON questao.id = alternativa.questao_id
    WHERE vestibular.nome = $1
    ORDER BY questao.id, alternativa.letra
    `,
    [vestibular]
  );

  return result.rows;
}

async function montarPorTopico(topico = "Cilindros") {
  const result = await pool.query(
    `
    SELECT
      questao.id,
      questao.enunciado,
      subtopico.nome AS topico,
      alternativa.letra,
      alternativa.texto
    FROM questao
    INNER JOIN subtopico
      ON questao.subtopico_id = subtopico.id
    INNER JOIN alternativa
      ON questao.id = alternativa.questao_id
    WHERE subtopico.nome = $1
    ORDER BY questao.id, alternativa.letra
    `,
    [topico]
  );

  return result.rows;
}

async function listarAnoRecente() {
  try {
    const result = await pool.query("SELECT * FROM view_filtro_ano_recente");
    return result.rows;
  } catch (erro) {
    if (!isUndefinedRelation(erro)) {
      throw erro;
    }
  }

  const result = await pool.query(
    `
    SELECT
      questao.id,
      questao.enunciado,
      vestibular.nome AS vestibular,
      vestibular.ano,
      alternativa.letra,
      alternativa.texto
    FROM questao
    INNER JOIN vestibular
      ON questao.vestibular_id = vestibular.id
    INNER JOIN alternativa
      ON questao.id = alternativa.questao_id
    ORDER BY vestibular.ano DESC
    `
  );

  return result.rows;
}

async function listarAnoAntigo() {
  try {
    const result = await pool.query("SELECT * FROM view_filtro_ano_antigo");
    return result.rows;
  } catch (erro) {
    if (!isUndefinedRelation(erro)) {
      throw erro;
    }
  }

  const result = await pool.query(
    `
    SELECT
      questao.id,
      questao.enunciado,
      vestibular.nome AS vestibular,
      vestibular.ano,
      alternativa.letra,
      alternativa.texto
    FROM questao
    INNER JOIN vestibular
      ON questao.vestibular_id = vestibular.id
    INNER JOIN alternativa
      ON questao.id = alternativa.questao_id
    ORDER BY vestibular.ano ASC
    `
  );

  return result.rows;
}

module.exports = {
  listar,
  montarPorDificuldade,
  montarPorVestibular,
  montarPorTopico,
  listarAnoRecente,
  listarAnoAntigo,
};
